...
function printUsage(err) {
  ...
  console.log(
    `node ${script} -o <output.hpp> -o <output.cpp> [-v] <animations.js>\n\n` +
    ...
    `-v                Verbose mode\n` +
    `<animations.js>   Source animations file`
  );
  if (err) {
    console.error(`\nError: %s`, err);
  }
...
for (let i = 0; i < args.length; i++) {
  if (args[i] === '-o') {
    ...
    if (ext === '.hpp') {
      if (outputHPP) {
        printUsage('Cannot specify multiple .hpp output files');
      }
      outputHPP = args[i];
    } else if (ext === '.cpp') {
      if (outputCPP) {
        printUsage('Cannot specify multiple .cpp output files');
      }
      outputCPP = args[i];
    }
  } else if (args[i] === '-v') {
...
// arguments look good, so add utility functions into global namespace
...
await import(pathToFileURL(inputFile));
