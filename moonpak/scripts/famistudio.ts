...
const isUnsignedEnvelope = (i: number) =>
  i == EnvelopeKindVolume || i == EnvelopeKindDutyCycle;

const envelopeKindToString = (i: number) => {
  switch (i) {
    case EnvelopeKindVolume: return 'Volume';
    case EnvelopeKindArpeggio: return 'Arpeggio';
    case EnvelopeKindPitchAbs: return 'PitchAbs';
    case EnvelopeKindPitchRel: return 'PitchRel';
    case EnvelopeKindDutyCycle: return 'DutyCycle';
    case EnvelopeKindWave: return 'Wave';
    case EnvelopeKindRepeat: return 'Repeat';
  }
  return `UnknownEnvelopeKind<${i}>`;
};
...
const channelKindToString = (i: number) => {
  switch (i) {
    case ChannelKindSine: return 'Sine';
    case ChannelKindTriangle: return 'Triangle';
    case ChannelKindSaw: return 'Saw';
    case ChannelKindSquare: return 'Square';
    case ChannelKindWave: return 'Wave';
    case ChannelKindPCM: return 'PCM';
    case ChannelKindNoise: return 'Noise';
  }
  return `UnknownChannelKind<${i}>`;
};
...
function noteToString(note: number) {
  const notes = ['C', 'C#', 'D', 'D#', 'E', 'F', 'F#', 'G', 'G#', 'A', 'A#', 'B'];
  const n = note % 12;
...
function printUsage(error?: string): never {
  console.log(
    ...
    'Commands:\n\n' +
    '* print <input.bin>\n\n' +
    '  Print instruments, songs, and events from a binary stream.\n\n' +
    '  <input.bin>      Input file generated by `song` command\n\n' +
  ...
  | ['dpcm-export', DpcmExportArgs]
{
  ...
  if ((cmd === 'song' || cmd === 'dpcm-export' || cmd === 'print') && inputFiles.length > 1) {
  ...
  if (cmd === 'print') {
    return ['print', { inputFile: inputFiles[0] }];
  } else if (cmd === 'song') {
    if (!dpcmTableFile) {
...
function printFile(data: Uint8Array) {
  let dataIndex = 0;
  const read8 = () => {
    if (dataIndex >= data.length) throw new Error('Invalid file');
  ...
  const magic = read32();
  console.log('Magic: "%s"',
    String.fromCharCode(magic & 0xff) +
    String.fromCharCode((magic >> 8) & 0xff) +
    String.fromCharCode((magic >> 16) & 0xff) +
    String.fromCharCode((magic >> 24) & 0xff)
  );

  const instrumentsLength = read16();
  const songsLength = read16();
  align32();
  const instrumentsOffset = read32();
  const songsOffset = read32();

  jumpTo(instrumentsOffset);
  const instrumentOffset = Array.from({ length: instrumentsLength }).map(() => read32());
  for (let ii = 0; ii < instrumentOffset.length; ii++) {
    console.log(`Instrument[${ii}]`);
    const instOffset = instrumentOffset[ii];
    jumpTo(instOffset);
    const envelopesLength = read8();
    if (envelopesLength <= 0) {
      console.log(`  - No envelopes`);
    }
    read8(); // reserved
    read8(); // reserved
    read8(); // reserved
    const envelopeOffset = Array.from({ length: envelopesLength }).map(() => read32());
    for (let ei = 0; ei < envelopeOffset.length; ei++) {
      const envOffset = envelopeOffset[ei];
      jumpTo(instOffset + envOffset);
      const envKind = read8();
      read8(); // reserved
      const envLoop = read16i();
      const envRelease = read16i();
      const valuesLength = read16();
      const values = Array.from({ length: valuesLength }).map(() =>
        isUnsignedEnvelope(envKind) ? read8() : read8i()
      );
      console.log(`  - ${ii}.Envelope[${ei}] Kind(${envelopeKindToString(envKind)}) ` +
        `Loop(${envLoop}) Release(${envRelease})`);
      for (let s = 0; s < values.length; s += 16) {
        console.log(`    ` +
          `${values.slice(s, s + 16).join(', ')}${s + 16 < values.length ? ',' : ''}`);
      }
    }
  }

  jumpTo(songsOffset);
  const songOffset = Array.from({ length: songsLength }).map(() => read32());
  for (let si = 0; si < songOffset.length; si++) {
    const soOffset = songOffset[si];
    jumpTo(soOffset);
    const loopChannelLength = read16i();
    const songLoop = loopChannelLength >> 4;
    const channelLength = (loopChannelLength & 15) + 1;
    const songLength = read16();
    console.log(`Song[${si}] Loop(${songLoop}) Length(${songLength})`);
    const channelOffset = Array.from({ length: channelLength }).map(() => read32());
    for (let ci = 0; ci < channelOffset.length; ci++) {
      const chanOffset = channelOffset[ci];
      jumpTo(chanOffset);
      const channelKind = read8();
      const initialVolume = read8();
      console.log(`  - ${si}.Channel[${ci}] Kind(${channelKindToString(channelKind)}) ` +
        `InitialVolume(${initialVolume})`);
      const patternsLength = read16();
      const instances = Array.from({ length: songLength }).map(() => read16i());
      console.log(`    - ${si}.${ci}.Instances:`);
      for (let s = 0; s < instances.length; s += 16) {
        console.log(`      ` +
      ...
      for (let pi = 0; pi < patternOffset.length; pi++) {
        ...
        for (;;) {
          ...
          events.push(ev);
          if (ev & 0x8000) {
            if (ev === EV_PATEND) break;
            continue;
          }
          const e2 = read16();
          events.push(e2);
        }
        console.log(`    - ${si}.${ci}.Pattern[${pi}] Events(${events.length * 2}B)`);
        let frame = 0;
        const log = (str: string) =>
          console.log(`      [${`000${frame}`.substr(-3)}] ${str}`);
        const hex = (n: number) => `0x${`000${n.toString(16)}`.substr(-4)}`;
        for (let ei = 0; ei < events.length; ei++) {
          const ev = events[ei];
          if (ev & 0x8000) {
            const param = ev & 0xff;
            switch (ev & 0xff00) {
              case EV_WAIT: log(`WAIT(${param + 1})`); frame += param + 1; break;
              case EV_PATEND & 0xff00:
                switch (param) {
                  case EV_PATEND & 0xff: log(`PATEND`); break;
                  case EV_NOINST & 0xff: log(`NOINST`); break;
                  case EV_STOP & 0xff: log(`STOP`); break;
                  default: log(hex(ev)); break;
                }
                break;
              case EV_NOINST: log(`NOINST`); break;
              case EV_STOP: log(`STOP`); break;
              case EV_INST1: log(`INST(${param})`); break;
              case EV_INST2: log(`INST(${param + 256})`); break;
              case EV_VOL: log(`VOL(${param})`); break;
              case EV_PATSKIP1: log(`PATSKIP(${param})`); break;
              case EV_PATSKIP2: log(`PATSKIP(${param + 256})`); break;
              default: log(hex(ev)); break;
            }
          } else {
            ei++;
            const e2 = events[ei];
            const note = ev >> 8;
            const autowait = (ev & 0x0080) !== 0 ? 1 : 0;
            const duration = e2 & 0x7ff;
            if (note < 108) {
              const attack = (ev & 0x0040) !== 0 ? 1 : 0;
              const release = (ev & 0x3f) | ((e2 >> 5) & 0x7c0);
              log(`${noteToString(note)} Duration(${duration}) Release(${release}) ` +
                `Wait(${autowait}) Attack(${attack})`);
            } else if (note === (EV_PCM >> 8)) {
              const index = (ev & 0x7f) | ((e2 >> 4) & 0xf80);
              log(`PCM Sample(${index}) Duration(${duration}) Wait(${autowait})`);
            } else {
              log(`${hex(ev)} / ${hex(e2)}`);
            }
            if (autowait) frame += duration;
          }
        }
      }
    }
...
switch (cmd) {
  case 'print': {
    ...
    printFile(await fs.readFile(inputFile));
    break;
  }
  case 'song': {
