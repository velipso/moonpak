...
json_value *json_parsevalue(const uint8_t *data, int *i);
void json_parseobject(json_value *v, const uint8_t *data, int *i);
void json_parsearray(json_value *v, const uint8_t *data, int *i);
char *json_parsestring(const uint8_t *data, int *i);
double json_parsenumber(const uint8_t *data, int *i);
void json_free(json_value *jv);
json_value *json_parsefp(FILE *fp);
json_value *json_parsefile(const char *file);
json_value *json_objectkey(json_value *v, const char *key);
json_value *json_objectkeyopt(json_value *v, const char *key);
  ...
  while (1) {
    ...
    if (JSON_IS_SPACE(ch)) {
      ...
      *i = *i + 1;
      ...
      v->u.object.count++;
      v->u.object.keys = (char **)realloc(v->u.object.keys, sizeof(void *) * v->u.object.count);
      v->u.object.keys[v->u.object.count - 1] = key;
      v->u.object.values =
        (struct json_value_st **)realloc(v->u.object.values, sizeof(void *) * v->u.object.count);
  ...
  while (1) {
    ...
    if (JSON_IS_SPACE(ch)) {
      *i = *i + 1;
      ...
      v->u.array.count++;
      v->u.array.values =
        (struct json_value_st **)realloc(v->u.array.values, sizeof(void *) * v->u.array.count);
...
void json_print(json_value *jv, int tab, int flags) {
    ...
    case JSON_ARRAY:
      if (jv->u.array.count <= 0) {
        ...
        for (int i = 0; i < jv->u.array.count; i++) {
          ...
          json_print(jv->u.array.values[i], tab, flags | JSON_SKIP_NEWLINE);
  ...
  if (tab == 0 && !(flags & JSON_SKIP_NEWLINE))
    printf("\n");
}
